...
# define Huxx wrk2
        if (.not.(ew_periodic)) then
          if (inode==0) then        ! Sort out bounding indices for
          ...
          if (inode==np_xi-1) then        ! are not available, so an
...
# define Hvxx wrk2
        ...
        uee(1-bf:nx+bf,1-bf:ny+bf) => scratch_2d(:,7)
        do j=jmin,jmax
          do i=1,nx
...
# define Huee wrk2
        ...
        vxx(1-bf:nx+bf,1-bf:ny+bf) => scratch_2d(:,7)
        do j=1,ny
          do i=imin,imax
